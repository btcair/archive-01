# EKS Version Upgrade Scenario

## 1. 개요
쿠버네티스(K8s)의 짧은 지원 주기(보통 14개월)에 맞춰 메이저/마이너 버전을 업그레이드할 때, **서비스 중단(Downtime)을 최소화하기 위한 배포 시나리오 및 아키텍처 전략**입니다.

## 2. 설명
* **In-Place Upgrade (인플레이스 업그레이드):**
  * 기존 클러스터의 컨트롤 플레인 버전을 올린 뒤, 워커 노드를 순차적으로 교체하는 방식입니다.
  * **장점:** 추가 리소스 비용이 적고 인프라 구조가 그대로 유지됩니다.
  * **단점:** 중간에 문제가 발생하면 롤백(Rollback)이 사실상 불가능합니다.
* **Blue/Green Upgrade (블루/그린 마이그레이션 - 무중단 권장):**
  * K8s 새 버전이 적용된 신규 클러스터(Green)를 별도의 VPC나 서브넷에 완전히 새로 구축합니다.
  * 애플리케이션을 양쪽에 모두 배포한 뒤, **Route 53(DNS) 또는 글로벌 로드밸런서**를 통해 트래픽을 구형(Blue)에서 신형(Green)으로 점진적(가중치 라우팅)으로 넘깁니다.
  * **장점:** 장애 발생 시 DNS 원복만으로 즉각적인 롤백이 가능하여 매우 안전합니다.

## 3. 참조 및 관련된 파일
* [[eks-version-upgrade]] (실제 업그레이드 절차)
* [[route53-vpc-resolver]] (Blue/Green 전환 시 DNS 제어)
* [[eks-ingress-nginx-retirement]] (버전 업그레이드 시 API 호환성 파악)

## 4. 트러블 슈팅
* **Blue/Green 시나리오 시 상태 유지(Stateful) 데이터 동기화 문제:**
  * DB나 캐시가 K8s 내부에 있다면, 신규 클러스터로 트래픽이 넘어가는 순간 세션이 끊어지거나 데이터 불일치가 발생합니다. 성공적인 Blue/Green을 위해서는 **상태 데이터를 K8s 외부(RDS, ElastiCache, S3)로 완전히 분리(Stateless 아키텍처)**해야 합니다.
* **사용 중단된 API(Deprecated API)로 인한 업그레이드 차단:**
  * K8s는 버전이 올라갈 때 구형 API(예: `v1beta1`)를 완전히 삭제합니다. 이 사실을 모르고 인플레이스 업그레이드를 진행하면 파드 배포가 전면 중단됩니다. `kube-no-trouble(kubent)`이나 `pluto` 같은 오픈소스 검사 툴로 사전 점검을 해야 합니다.
